...
export default function LookbookDetailsPage() {
  ...
  // Interactive state — all of this is persisted to localStorage so it
  // survives navigating away and back. See notes at the end for how this
  // should move to real DB-backed endpoints.
  const [isFollowing, setIsFollowing] = useState(false);
  const [isLiked, setIsLiked] = useState(false);
  const [baseLikes, setBaseLikes] = useState(DEFAULT_LIKES);
    ...
    const fetchLookbookDetails = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          resolved = data.lookbook || data.data || data;
        } else {
      ...
      setLookbook(resolved);

      // --- Restore persisted interaction state ---
      const creatorKey = resolved?.creator?.username || resolved?.creator?.name || 'creator';

      setBaseLikes(resolved?.likes_count ?? DEFAULT_LIKES);
      setIsLiked(localStorage.getItem(`tribe_liked_${id}`) === 'true');
      setIsFollowing(localStorage.getItem(`tribe_following_${creatorKey}`) === 'true');

      const storedComments = localStorage.getItem(`tribe_comments_${id}`);
      if (storedComments) {
        try {
        ...
        const initialComments = Array.isArray(resolved?.comments) && resolved.comments.length > 0
          ? resolved.comments
          : DEFAULT_COMMENTS;
        setComments(initialComments);
        localStorage.setItem(`tribe_comments_${id}`, JSON.stringify(initialComments));
      }
  ...
  const handleLike = () => {
    const next = !isLiked;
    setIsLiked(next);
    if (id) localStorage.setItem(`tribe_liked_${id}`, String(next));
    // TODO(backend): POST /api/lookbooks/:id/like (toggle) so this survives
    // across devices/browsers, not just this browser's localStorage.
  };

  const handleFollowToggle = () => {
    const next = !isFollowing;
    setIsFollowing(next);
    const creatorKey = lookbook?.creator?.username || lookbook?.creator?.name || 'creator';
    localStorage.setItem(`tribe_following_${creatorKey}`, String(next));
  ...
  const handlePostComment = (e: React.FormEvent) => {
    ...
    const updated = [{ id: Date.now(), user: myUsername, text: newComment, time: "Just now" }, ...comments];
    setComments(updated);
    if (id) localStorage.setItem(`tribe_comments_${id}`, JSON.stringify(updated));
    setNewComment("");
    // TODO(backend): POST /api/lookbooks/:id/comments { text }
  };
  ...
  const creatorName = lookbook.creator?.username || lookbook.creator?.name || "creator";
      ...
      <div className="max-w-[1200px] mx-auto px-6 pt-28">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-lg leading-tight">@{creatorName}</h2>
          ...
          <div className="lg:col-span-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              {lookbook.products?.map((product: any, idx: number) => {
                const image = typeof product === 'string' ? PRODUCT_IMAGE_FALLBACK : (product.image_url || PRODUCT_IMAGE_FALLBACK);
                const name = typeof product === 'string' ? 'Myntra Item' : (product.name || product.full_name || 'Myntra Item');
                const price = typeof product === 'string' ? '999' : (product.price ?? '999');
                return (
                  <div key={idx} className="group bg-white p-3 rounded-[1.5rem] shadow-sm hover:shadow-xl transition-all border border-black/5">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-sm text-[#888888] font-medium">₹{price}</p>
                      <button
